...
export class ViewEventsComponent implements OnInit {
  ...
  eventList: any[] = [];
  allEvents: any[] = []; // NEW: Store all events here
  minDate: string;
  ...
  // Increase items per page to show more events at once
  itemsPerPage: number = 10; // CHANGED from 3 to 10
  totalPages: number = 1;
  ...
  getEvents() {
    this.httpService.GetEvents().subscribe(
      (data) => {
        this.allEvents = data; // CHANGED: Store original data
        this.eventList = [...data]; // CHANGED: Copy to eventList
  ...
  // FIXED: Filter from allEvents instead of modifying eventList
  filterPastEvents(): void {
    const currentDate = new Date();
    this.eventList = this.allEvents.filter(event => new Date(event.dateTime) < currentDate);
    this.currentPage = 1;
    this.totalPages = Math.ceil(this.eventList.length / this.itemsPerPage);
    this.setPaginatedEvents();
  }
  ...
  viewAllEvents(): void {
    this.eventList = [...this.allEvents]; // FIXED: Restore from allEvents
    this.currentPage = 1;
    this.totalPages = Math.ceil(this.eventList.length / this.itemsPerPage);
    this.setPaginatedEvents();
  }
